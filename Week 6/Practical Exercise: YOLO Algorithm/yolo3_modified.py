...
def draw_boxes(image_filename, boxes, class_colors=None, class_styles=None, figsize=(12, 8)):
    ...
    for box in boxes:
        ...
        if class_colors and box.label in class_colors:
            color = class_colors[box.label]
        else:
            color = 'blue'  # Default color
        
        # Choose linestyle based on class label
        if class_styles and box.label in class_styles:
            linestyle = class_styles[box.label]
        else:
            linestyle = 'solid'  # Default linestyle
        
        # Draw box
        rect = Rectangle((x, y), w, h, fill=False, color=color, linewidth=2, linestyle=linestyle)
        ax.add_patch(rect)
        
        # Add label
        label = f'{box.label} ({box.score:.0%})'
        ax.text(x, y, label, color=color, fontsize=12, backgroundcolor='white')
